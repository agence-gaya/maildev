...
mailServer.relayMail = function (host, idOrMailObject, isAutoRelay, done) {
  ...
  // isAutoRelay is an optional argument
  if (typeof isAutoRelay === 'function') {
    done = isAutoRelay
    isAutoRelay = false
  }

  // If we receive an email id, get the email object
          ...
          files.forEach(function (file) {
            ...
            if (path.parse(file).ext === '.evl') {
              fs.readFile(filePath, 'utf8', function(err, data) {
                const filePathEvl = filePath
                const filePathEml = path.join(persistencePath, dir, path.parse(file).name + '.eml')
                if (err) {
                  logger.error('Ignore envelope %s, loading failed: %s', filePathEvl, err)
                  return
                }
                const host = mailServer.virtualHostFromDir(dir)
                const envelope = (function(data) {
                  try {
                    return JSON.parse(data)
                  } catch (err) {
                    logger.error('Ignore envelope %s, parsing failed: %s', filePathEvl, err)
                    return false
                  }
                })(data)
                if (!envelope) {
                ...
                envelope.host = host
                envelope.date = fs.statSync(filePathEml).ctime
                envelope.init = true

                fs.readFile(filePathEml, 'utf8', function (err, data) {
                  if (err) {
                    logger.error('Ignore mail %s, loading failed: %s', filePath, err)
                    return
                  }
