            ...
            files.forEach(function (file) {
              ...
              if (path.parse(file).ext === '.eml') {
                fs.readFile(filePath, 'utf8', function (err, data) {
                  ...
                  logger.log('Restore mail %s, id: %s', host, idMail)
                  ...
                  parseStream.on(
                    'end',
                    function (mail) {
                      // Construct envelope from content of mail file
                      // This is not the true original envelope of the received mail
                      // In some case end user can be confused by envelope change by this
                      // @todo serialize somewhere the original envelope to restore it
                      const envelope = {
                        from: mail.from[0],
                        to: mail.to,
                        host: host,
