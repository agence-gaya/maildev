...
mailServer.deleteOldEmail = function (delay) {
  var expired = new Date().getTime() - delay
  store.forEach(function (element, index) {
  	var stats = fs.statSync(path.join(mailServer.mailDir, element.id + '.eml'))
    if (stats.ctime.getTime() < expired) {
        mailServer.deleteEmail(element.id, function() {})
    }
  })
}
